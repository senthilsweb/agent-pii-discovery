...
- [x] **Cache-hit observability fixed 2026-08-08** (real gap against PRD §10.3, found while building Arize dashboard guidance): a cache hit created no session, so it left zero trace anywhere — no span, no new DB row. Added a `cache_hits` table (`db.record_cache_hit()`, `db.cache_hit_rate()` — computable locally, no Arize dependency) and a purpose-built minimal trace (`client/forwarder.py forward_cache_hit()`, same `pii_scan.session` span name so one dashboard dimension covers both fresh scans and cache hits). Verified live and free: `client.scan` on an already-cached fixture recorded the DB row and forwarded the trace with zero session cost, zero tokens — `cache_hit_rate()` correctly reported 25% (1 of 4 requests). 6 new tests, 110 total green.
